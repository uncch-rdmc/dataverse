...
class test_root_dataverse(unittest.TestCase):
    ...
    def test_test_root_dataverse(self):
        ...
        if not ("Create Root Dataverse" in wd.find_element(By.TAG_NAME, "html").text):
        ...
        wd.find_element(By.ID, "dataverseForm:name").send_keys("root dv")
        wd.find_element(By.ID, "dataverseForm:alias").click()
        wd.find_element(By.ID, "dataverseForm:alias").clear()
        wd.find_element(By.ID, "dataverseForm:alias").send_keys("rootdv")
        ...
        time.sleep(1)
        if not ("root dv" in wd.find_element(By.TAG_NAME, "html").text):
            success = False
